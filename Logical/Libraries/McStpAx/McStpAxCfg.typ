TYPE
	...
	END_STRUCT;
	McSAMRotDirEnum :
		( (*Rotational direction of motor current*)
		mcSAMRD_STD := 0, (*Standard - According to the motor wiring*)
		mcSAMRD_INV := 1 (*Inverse - Opposite to the motor wiring*)
		);
	McSAMType : STRUCT (*Parameter to handle the motor*)
		SteppingAngle : REAL; (*Stepping angle of the motor [°]*)
		RotationDirection : McSAMRotDirEnum; (*Rotational direction of motor current*)
	END_STRUCT;
	McSAELEnum :
	...
	McSACCSCCSCCCurRedEnum :
		...
		mcSACCSCCSCCCR_RED_50 := 0, (*Reduction 50 - Maximum reduction to 50%*)
		mcSACCSCCSCCCR_RED_25 := 1 (*Reduction 25 - Maximum reduction to 25%*)
		);
	McSACCSCCSCCRedActLvlEnum :
		( (*Current is decreased when the Motor load value is greater than or equal to this value as many times as it is configured at Reduction activation delay. This value should be greater than the 'Reduction reset level' by a maximum of 512. The value of this parameter should not be set lower than the 'Reduction reset level'.*)
	...
	McSACCSCCSCCRedRstLvlEnum :
		( (*Current is increased when the Motor load value is less than or equal to this value as many times as it is configured at Reduction reset delay. The value of this parameter should not be set higher than the 'Reduction activation level'.*)
	...
	McSACCSCCSCCType : STRUCT (*Configuration for load dependent current control*)
		...
		ReductionActivationLevel : McSACCSCCSCCRedActLvlEnum; (*Current is decreased when the Motor load value is greater than or equal to this value as many times as it is configured at Reduction activation delay. This value should be greater than the 'Reduction reset level' by a maximum of 512. The value of this parameter should not be set lower than the 'Reduction reset level'.*)
		...
		ReductionResetLevel : McSACCSCCSCCRedRstLvlEnum; (*Current is increased when the Motor load value is less than or equal to this value as many times as it is configured at Reduction reset delay. The value of this parameter should not be set higher than the 'Reduction activation level'.*)
	...
	McSACMPACCtrlPosAutAdjEnum :
		( (*Automatic adjustment selector setting*)
		mcSACMPACCPAA_NOT_USE := 0, (*Not used - Manipulated variable not adjusted*)
		mcSACMPACCPAA_VELBASED := 1 (*Velocity-based - Reduces the effect of the manipulated variable based on the set velocity*)
		);
	McSACMPACCtrlPosAutAdjVelBsdType : STRUCT (*Type mcSACMPACCPAA_VELBASED settings*)
		Level : REAL; (*Velocity from which the effect of the manipulated variable is adjusted [Measurement units/s]*)
		Zone : REAL; (*Velocity range in which the effect of the manipulated variable is linearly reduced [Measurement units/s]*)
	END_STRUCT;
	McSACMPACCtrlPosAutAdjType : STRUCT (*Adjusts the effect of the position controller's manipulated variable*)
		Type : McSACMPACCtrlPosAutAdjEnum; (*Automatic adjustment selector setting*)
		VelocityBased : McSACMPACCtrlPosAutAdjVelBsdType; (*Type mcSACMPACCPAA_VELBASED settings*)
	END_STRUCT;
	McSACModPosAndCurCtrlPosType : STRUCT (*Position controller parameters*)
		...
		AutomaticAdjustment : McSACMPACCtrlPosAutAdjType; (*Adjusts the effect of the position controller's manipulated variable*)
	END_STRUCT;
	...
	McSAHModClrPosErrEnum :
		( (*Clear the lag error during the homing procedure*)
		mcSAHMCPE_NO := 0, (*No - mcSWITCH_OFF*)
		mcSAHMCPE_YES := 1 (*Yes - mcSWITCH_ON*)
		);
	McSAHType : STRUCT (*Homing mode and parameters which can be used within the application program as preconfigured setting*)
		...
		ClearPositionError : McSAHModClrPosErrEnum; (*Clear the lag error during the homing procedure*)
	END_STRUCT;
	McSASRDEEnum :
	...
	McSAZVFEnum :
		...
		mcSAZVF_NOT_USE := 0, (*Not used - No zero vibration filter is applied*)
		mcSAZVF_USE := 1 (*Used - Zero vibration filter is applied*)
		);
	McSAZVFUseType : STRUCT (*Type mcSAZVF_USE settings*)
	...
	END_STRUCT;
	McSAZVFType : STRUCT (*Zero vibration filter*)
		Type : McSAZVFEnum; (*Zero vibration filter selector setting*)
		Used : McSAZVFUseType; (*Type mcSAZVF_USE settings*)
	END_STRUCT;
	McSADIAllSrcEnum :
		...
		mcSADIAS_FOR_BY_FUN_BLK := 11 (*Force by function block -*)
		);
	...
	McSSLdSimModEnum :
		...
		mcSSLSM_NOT_USE := 0, (*Not used - Load simulation is not switched on during the axis initialization*)
		mcSSLSM_SET_VAL_GEN := 1 (*Set value generation - Only set value generation is active, no controlled system is simulated*)
		);
	McSSLdSimModType : STRUCT (*Mode for the motor and load simulation*)
		Type : McSSLdSimModEnum; (*Load simulation mode selector setting*)
	END_STRUCT;
	McSSType : STRUCT (*Parameters which influence the simulation possibilities of this axis*)
		LoadSimulationMode : McSSLdSimModType; (*Mode for the motor and load simulation*)
	END_STRUCT;
	...
	McCfgStpAxType : STRUCT (*Main data type corresponding to McCfgTypeEnum mcCFG_STP_AX*)
		...
		ZeroVibrationFilter : McSAZVFType; (*Zero vibration filter*)
		...
		Simulation : McSSType; (*Parameters which influence the simulation possibilities of this axis*)
	...
	McCfgStpAxJerkFltrType : STRUCT (*Main data type corresponding to McCfgTypeEnum mcCFG_STP_AX_JERK_FLTR*)
		JerkFilter : McSAJFType; (*Jerk filter*)
	END_STRUCT;
	McCfgStpAxZeroVibFltrType : STRUCT (*Main data type corresponding to McCfgTypeEnum mcCFG_STP_AX_ZERO_VIB_FLTR*)
		ZeroVibrationFilter : McSAZVFType; (*Zero vibration filter*)
	END_STRUCT;
	...
	McCfgStpAxDigOutType : STRUCT (*Main data type corresponding to McCfgTypeEnum mcCFG_STP_AX_DIG_OUT*)
		DigitalOutputs : McSADOType; (*Various digital output functionalities e.g. like brake*)
	END_STRUCT;
	McCfgStpAxSimType : STRUCT (*Main data type corresponding to McCfgTypeEnum mcCFG_STP_AX_SIM*)
		Simulation : McSSType; (*Parameters which influence the simulation possibilities of this axis*)
	END_STRUCT;
	...
	McSMActModSimOnPLCEnum :
		...
		mcSMAMSOP_OFF := 0, (*Off - The module is not simulated on the PLC*)
		mcSMAMSOP_ON := 1 (*On - The module is simulated on the PLC*)
		);
	McCfgStpModType : STRUCT (*Main data type corresponding to McCfgTypeEnum mcCFG_STP_MOD*)
		ActivateModuleSimulationOnPLC : McSMActModSimOnPLCEnum; (*Activates or deactivates the module simulation on the PLC*)
	END_STRUCT;
END_TYPE